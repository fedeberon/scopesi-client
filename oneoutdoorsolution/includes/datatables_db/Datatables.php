    ...
    */
    protected function input($field, $escape = TRUE)
    {
		...
		if(isset($_POST['sEcho']) && isset($_POST[$field]))
        	return ($escape == TRUE)? $_POST[$field] : $_POST[$field];
      	elseif(isset($_GET['sEcho']) && isset($_GET[$field]))
        	return ($escape == TRUE)? $_GET[$field] : $_GET[$field];
      	else
        	return FALSE;
    }
    ...
    protected function get_ordering()
    {
      ...
      for($i = 0; $i < intval($this->input('iSortingCols')); $i++)
        if(isset($mColArray[intval($this->input('iSortCol_' . $i))]) && in_array($mColArray[intval($this->input('iSortCol_' . $i))], $columns ) && $this->input('bSortable_'.intval($this->input('iSortCol_' . $i))) == 'true' ) {
			if(strpos($mColArray[intval($this->input('iSortCol_' . $i))], ' ')>0)
            	$orderBy = substr($mColArray[intval($this->input('iSortCol_' . $i))], 0, strpos($mColArray[intval($this->input('iSortCol_' . $i))], ' '));
			else
				$orderBy = $mColArray[intval($this->input('iSortCol_' . $i))];
        	$this->ar->order_by($orderBy, $this->input('sSortDir_' . $i));
        }
    }
      ...
      if($sSearch != '')
        for($i = 0; $i < count($mColArray); $i++) {
          if($this->input('bSearchable_' . $i) == 'true' && in_array($mColArray[$i], $columns)) {
          	if(strpos($this->select[$mColArray[$i]], ' ')>0)
            	$sWhere .= substr($this->select[$mColArray[$i]], 0, strpos($this->select[$mColArray[$i]], ' '))  . " LIKE '%" . $sSearch . "%' OR ";
			else
